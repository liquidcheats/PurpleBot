...
library.theme = {
    ...
    backgroundcolor = Color3.fromRGB(14, 14, 15),
    tabstextcolor = Color3.fromRGB(255, 255, 255),
    bordercolor = Color3.fromRGB(30, 30, 30),
    accentcolor = Color3.fromRGB(110, 103, 143),
    accentcolor2 = Color3.fromRGB(110, 103, 143),
    outlinecolor = Color3.fromRGB(35, 35, 35),
    outlinecolor2 = Color3.fromRGB(30, 30, 30),
    sectorcolor = Color3.fromRGB(25, 25, 25),
    toptextcolor = Color3.fromRGB(200, 200, 200),
    topheight = 45,
    topcolor = Color3.fromRGB(25, 25, 23),
...
runservice.Heartbeat:Connect(function()
    ...
    if watermark.text:find("{fps}") or watermark.text:find("{ping}") then
        ...
        if currenttime - starttime >= 1 then
            ...
            local success, response = pcall(function()
                local start = tick()
                httpservice:GetAsync("https://www.roblox.com") -- Replace "https://example.com" with a real HTTPS URL
                local finish = tick()
                ping = math.floor((finish - start) * 1000)
            end)

            local updatedText = watermark.text:gsub("{fps}", fps .. " FPS"):gsub("{ping}", ping .. " ms")
            watermark.label.Text = updatedText

            watermark.label.Size = UDim2.new(0, watermark.label.TextBounds.X + 10, 0, 25)
            watermark.mainbar.Size = UDim2.new(0, watermark.label.TextBounds.X + 4, 0, 25)
            watermark.topbar.Size = UDim2.new(0, watermark.label.TextBounds.X + 4, 0, 1)
...
function library:Notify(text, mode, time, position)
    local player = game:GetService("Players").LocalPlayer
    local playerGui = player:WaitForChild("PlayerGui")
    local notifymain = Instance.new("ScreenGui", playerGui)

    if syn then
        syn.protect_gui(notifymain)
    end

    local notifyFrame = Instance.new("Frame", notifymain)
    notifyFrame.Size = UDim2.new(0, 285, 0, 20.4)
    notifyFrame.Position = UDim2.new(0, -285, position.Y, 0)
    notifyFrame.BorderSizePixel = 1.5
    notifyFrame.BorderColor3 = Color3.fromRGB(255, 255, 255)
    notifyFrame.Visible = true

    local frameGradient = Instance.new("UIGradient", notifyFrame)
    ...
    textLabel.BackgroundColor3 = Color3.new(35, 35, 35)
    textLabel.BackgroundTransparency = 1
    textLabel.TextXAlignment = Enum.TextXAlignment.Left

    local uiPadding = Instance.new("UIPadding", textLabel)
    uiPadding.PaddingLeft, uiPadding.PaddingTop, uiPadding.PaddingRight, uiPadding.PaddingBottom = UDim.new(0, 6), UDim.new(0, 6), UDim.new(0, 7), UDim.new(0, 5)
    ...
    local function closeNotify()
        local hideTween = tweenservice:Create(notifyFrame, TweenInfo.new(0.5), { Position = UDim2.new(0, -285, position.Y, 0) })
        hideTween:Play()
    ...
    delay(time or 1, function()
        closeNotify()
        table.remove(library.activeNotifications, 1)
    end)

    local timeRemaining = time or 1
    local updateTimeBar = tweenservice:Create(timeBar, TweenInfo.new(timeRemaining, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), { Size = UDim2.new(0, 0, 0, 3) })
    updateTimeBar:Play()

    for _, notify in ipairs(library.activeNotifications) do
        local existingTween = tweenservice:Create(notify.Frame, TweenInfo.new(0.5), { Position = notify.Position })
        existingTween:Play()
        notify.Position = UDim2.new(0, notify.Position.X.Offset, notify.Position.Y.Offset - 30)
    end

    table.insert(library.activeNotifications, 1, { Frame = notifyFrame, Position = UDim2.new(0, position.X, position.Y, 0) })

    return notifyFrame
end

function Indicators(visible, v1, v2)
